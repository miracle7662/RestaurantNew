...
-- NEW: mixed-selection support (some selected rooms already checked out,
-- some still active, in the SAME checkout call)
...
-- 3b. NEW: split the explicit room selection into "already active" vs
-- "already checked out" so we can detect a mixed selection.
IF v_selected_room_ids_all IS NOT NULL AND v_selected_room_ids_all != '' THEN
    SELECT GROUP_CONCAT(DISTINCT room_id)
    ...
    IF v_selected_active_room_ids IS NOT NULL AND v_selected_active_room_ids != ''
...
IF v_merge_mode = 1 THEN
    ...
    -- =============================================================
    -- Re-insert ALL folio records from checkin_guest_folio_master
    -- =============================================================
...
IF v_split_mode = 1 THEN
    ...
    SELECT
        ...
        'checked_out', v_checkout_dt, v_user_id, COALESCE(p_checkout_reason, 'Split checkout'),
    ...
    -- =============================================================
    -- Insert fresh folio records for new checkout
    -- =============================================================
    ...
    -- =============================================================
    -- Re-insert folio records for source checkout (remaining rooms)
    -- =============================================================
...
-- =============================================================
-- Insert ALL folio records including room_id = NULL
-- =============================================================
INSERT INTO Checkout_Folio_Master (
    checkin_id, checkout_id, hotel_id, detail_id, room_id,
    transaction_type, transaction_datetime,
...
--   END IF;
-- This caused every second/third individual room checkout to collapse
-- into a single Checkout_Master row, breaking individual checkout.
-- It has been deleted entirely. Individual checkouts now stay as separate
-- Checkout_Master rows. Merging is only performed explicitly via
-- Section 7 (MERGE MODE, v_merge_mode = 1), which is unchanged.

-- =================================================================
-- 9b. NEW: MIXED-SELECTION MERGE
-- Only runs when THIS call's room selection explicitly contained BOTH
-- already-checked-out rooms and still-active rooms (v_mixed_mode = 1),
-- e.g. 4 rooms checked in, 2 already checked out individually, then
-- user selects all 4 and checks out again. The active rooms were just
-- checked out above (new Checkout_Master row = v_checkout_id). Now we
-- combine that new row with the pre-existing Checkout_Master row(s) for
-- the previously checked-out selected rooms into a single row, exactly
-- like Section 7's merge, but scoped to only the selected rooms instead
-- of every checked-out room on the checkin.
-- =================================================================
IF v_mixed_mode = 1 THEN
    SET v_debug_msg = CONCAT('Mixed-selection merge: checkout datetime = ', v_checkout_dt);

    SET v_all_checked_out_room_ids = v_selected_room_ids_all;

    OPEN merge_cursor;
    SET v_done = 0;
    SET v_keeper_checkout_id = 0;

    read_loop_mixed: LOOP
        FETCH merge_cursor INTO v_cur_checkout_id;
        IF v_done THEN
            LEAVE read_loop_mixed;
        END IF;
        IF v_keeper_checkout_id = 0 THEN
            SET v_keeper_checkout_id = v_cur_checkout_id;
        ELSE
