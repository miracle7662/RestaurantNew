...
sp_perform_checkout: BEGIN
    ...
    -- NEW: variables for existing checkout lookup and skip insert flag
    DECLARE v_existing_checkout_id_for_update INT DEFAULT 0;
    DECLARE v_existing_ldg_bill_no_for_update VARCHAR(50) DEFAULT '';
    DECLARE v_skip_insert TINYINT DEFAULT 0;
    ...
    -- ============================================================================
    -- CASE 6: UNDO CHECKOUT FOR SPECIFIC ROOMS (unchanged)
    -- ============================================================================
    ...
    -- ============================================================================
    -- MULTI-BILL: Skip merge logic when p_bill_no > 0
    -- ============================================================================
    IF p_bill_no = 0 THEN

        -- ============================================================================
        -- CASE 4: Merge Before Checkout (unchanged)
        -- ============================================================================
        ...
        -- ============================================================================
        -- CASE 5: Merge After Full Checkout (unchanged)
        -- ============================================================================
        IF v_active_room_count = 0 
           AND v_checkout_master_count > 1 
           AND v_selected_rooms_count = v_total_rooms_in_checkin THEN
    ...
    -- ============================================================================
    -- CHECK FOR RE-CHECKOUT (unchanged)
    -- ============================================================================
    ...
    -- ============================================================================
    -- MULTI-BILL: Conditionally mark rooms as checked out
    -- ============================================================================
    IF (p_is_last_bill = 1 OR p_bill_no = 0) THEN
    ...
    -- ============================================================================
    -- NORMAL CHECKOUT – with bill_no filter
    -- ============================================================================
    SELECT
        COALESCE(SUM(room_tariff), 0),
    ...
    WHERE checkin_id = p_checkin_id
      AND FIND_IN_SET(room_id, v_active_room_ids) > 0
      AND (p_bill_no = 0 OR bill_no = p_bill_no);
    ...
    SELECT COALESCE(JSON_ARRAYAGG(room_number), JSON_ARRAY())
    INTO v_processed_rooms_json
    FROM (SELECT DISTINCT room_number FROM checkin_detail_master 
          WHERE checkin_id = p_checkin_id 
            AND FIND_IN_SET(room_id, v_active_room_ids) > 0
            AND (p_bill_no = 0 OR bill_no = p_bill_no)) d;

    SELECT COALESCE(JSON_ARRAYAGG(room_id), JSON_ARRAY())
    INTO v_processed_room_ids_json
    FROM (SELECT DISTINCT room_id FROM checkin_detail_master 
          WHERE checkin_id = p_checkin_id 
            AND FIND_IN_SET(room_id, v_active_room_ids) > 0
            AND (p_bill_no = 0 OR bill_no = p_bill_no)) d;
    ...
    -- MULTI-BILL: filter folio entries by bill_no
    SELECT
        COALESCE(SUM(CASE WHEN transaction_type IN ('Booking Receipt','Advance Addition') THEN credit_amount ELSE 0 END), 0),
        COALESCE(SUM(CASE WHEN transaction_type = 'CHARGE' THEN debit_amount ELSE 0 END), 0),
        COALESCE(SUM(CASE WHEN transaction_type = 'ALLOWANCE' THEN credit_amount ELSE 0 END), 0)
    ...
    -- Remaining active rooms – only compute if this is the last bill or legacy
    IF (p_is_last_bill = 1 OR p_bill_no = 0) THEN
        SELECT COUNT(*) INTO v_remaining_active
        FROM checkin_detail_master
        WHERE checkin_id = p_checkin_id AND is_checkout = 0;
    ELSE
        SET v_remaining_active = 1;
    END IF;

   -- ============================================================================
-- Check if a checkout master already exists for this bill
-- ============================================================================
IF p_bill_no > 0 THEN
    SELECT checkout_id, ldg_bill_no
    INTO v_existing_checkout_id_for_update, v_existing_ldg_bill_no_for_update
    FROM Checkout_Master
    WHERE checkin_id = p_checkin_id
      AND bill_no = p_bill_no
    LIMIT 1;
ELSEIF p_invoice_no IS NOT NULL AND p_invoice_no != '' THEN
    SELECT checkout_id, ldg_bill_no
    INTO v_existing_checkout_id_for_update, v_existing_ldg_bill_no_for_update
    FROM Checkout_Master
    WHERE checkin_id = p_checkin_id
      AND ldg_bill_no = p_invoice_no
    LIMIT 1;
ELSE
    -- अगर कोई identifier नहीं दिया, तो सबसे नया partial_checkout लें
    SELECT checkout_id, ldg_bill_no
    INTO v_existing_checkout_id_for_update, v_existing_ldg_bill_no_for_update
    FROM Checkout_Master
    WHERE checkin_id = p_checkin_id
      AND status = 'partial_checkout'
    ORDER BY checkout_id DESC
    LIMIT 1;
END IF;

IF v_existing_checkout_id_for_update > 0 THEN
    SET v_skip_insert = 1;
    SET v_checkout_id = v_existing_checkout_id_for_update;
    SET v_ldg_bill_no = v_existing_ldg_bill_no_for_update;
END IF;

    -- ============================================================================
    -- Insert or Update Checkout_Master (modified for update support)
    -- ============================================================================
    IF v_is_re_checkout = 0 THEN
        IF v_skip_insert = 0 THEN
            ...
            SELECT
                ...
                CASE 
                    WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active = 0 THEN 'checked_out'
                    WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active > 0 THEN 'partial_checkout'
                    ELSE 'pending'
                END,
                v_checkout_dt,
                v_user_id,
                COALESCE(p_checkout_reason, 'Regular checkout'),
                CASE WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active > 0 THEN 1 ELSE 0 END,
        ...
        ELSE
            ...
            -- Delete old details, folio, and room charges
            DELETE FROM Checkout_Detail WHERE checkout_id = v_checkout_id;
            DELETE FROM Checkout_Folio_Master WHERE checkout_id = v_checkout_id;
            DELETE FROM Checkout_Room_Charges WHERE checkout_id = v_checkout_id;

            -- Re-insert details (same as below, but done here)
            INSERT INTO Checkout_Detail (
                checkin_id, checkout_id, hotelid, room_id, room_number,
                room_category_id, room_category_name,
                converted_category_id, converted_category_name,
            ...
            WHERE cgfm.checkin_id = p_checkin_id
              AND (
                  (p_bill_no = 0 AND (cgfm.room_id IS NULL OR FIND_IN_SET(cgfm.room_id, v_active_room_ids) > 0))
                  OR
                  (p_bill_no > 0 AND cgfm.bill_no = p_bill_no)
              );
            ...
            WHERE cgrc.checkin_id = p_checkin_id
              ...
              AND EXISTS (
                  SELECT 1 FROM checkin_detail_master cdm
                  WHERE cdm.checkin_id = cgrc.checkin_id
                    AND cdm.room_id = cgrc.room_id
                    AND (p_bill_no = 0 OR cdm.bill_no = p_bill_no)
              );

            -- Update master with final totals
            UPDATE Checkout_Master
            SET
                tot_room_tariff = v_room_tariff_sum,
                tot_ex_pax_charge = v_ex_pax_charge,
                ...
                updated_date = v_now,
                checkout_date = v_checkout_dt,
                is_partial_checkout = CASE 
                    WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active > 0 THEN 1 
                    ELSE 0 
                END,
                status = CASE 
                    WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active = 0 THEN 'checked_out'
            ...
            SET v_debug_msg = CONCAT('Updated existing checkout_id: ', v_checkout_id, ', ldg_bill_no: ', v_ldg_bill_no);
        END IF;
    ELSE
        -- Re-checkout: update existing checkout (this block remains unchanged)
        SET v_checkout_id = v_existing_checkout_id;
        SET v_ldg_bill_no = v_existing_ldg_bill_no;
    ...
    -- ============================================================================
    -- Insert Checkout_Detail – with bill_no (only if not skipped)
    -- ============================================================================
    IF v_skip_insert = 0 THEN
        INSERT INTO Checkout_Detail (
            checkin_id, checkout_id, hotelid, room_id, room_number,
        ...
        -- ============================================================================
        -- Insert Folio records – with bill_no
        -- ============================================================================
        INSERT INTO Checkout_Folio_Master (
            checkin_id, checkout_id, hotel_id, detail_id, room_id,
            transaction_type, transaction_datetime,
        ...
        -- ============================================================================
        -- Insert Room Charges – join with checkin_detail_master to filter by bill_no
        -- ============================================================================
        INSERT INTO Checkout_Room_Charges (
            checkin_id, checkout_id, guest_id, room_id, category_id,
            pax_count, pax_price, pax_tax,
    ...
    -- ============================================================================
    -- Update CheckIn_Master if all rooms are checked out (only on last bill or legacy)
    -- ============================================================================
    IF (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active = 0 THEN
        UPDATE CheckIn_Master
        SET status = 'checked_out', updated_by_id = v_user_id, updated_date = v_now
        WHERE checkin_id = p_checkin_id;
    END IF;
    ...
    -- ============================================================================
    -- Update room status (only on last bill or legacy)
    -- ============================================================================
    IF (p_is_last_bill = 1 OR p_bill_no = 0) AND v_room_ids_to_update IS NOT NULL AND v_room_ids_to_update != '' THEN
        UPDATE room_master
        SET room_status_id = 7, updated_by_id = v_user_id, updated_date = v_now
        WHERE FIND_IN_SET(room_id, v_room_ids_to_update);
    END IF;
    ...
    -- ============================================================================
    IF v_is_re_checkout = 0 AND v_checkout_id IS NOT NULL AND v_skip_insert = 0 THEN
        UPDATE Checkout_Master
        SET
            tot_room_tariff = v_room_tariff_sum,
            tot_ex_pax_charge = v_ex_pax_charge,
    ...
    -- ============================================================================
    -- HANDLE MIXED MODE MERGE – only when p_bill_no = 0
    -- ============================================================================
    IF p_bill_no = 0 AND v_mixed_mode = 1 THEN
    ...
    -- ============================================================================
    -- FINAL COMMIT AND RESPONSE – includes bill_no info
    -- ============================================================================
    COMMIT;
    ...
    SELECT JSON_OBJECT(
        ...
        'message', CASE 
            ...
            WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active = 0 THEN 'Full checkout completed'
            WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active > 0 THEN CONCAT('Partial checkout completed. ', v_remaining_active, ' room(s) remain active.')
            ELSE CONCAT('Intermediate bill (bill_no=', p_bill_no, ') processed. Rooms remain active.')
        END,
        'checkout_id', COALESCE(v_checkout_id, 0),
        'checkin_id', p_checkin_id,
        'is_partial', CASE WHEN (p_is_last_bill = 1 OR p_bill_no = 0) AND v_remaining_active > 0 THEN 1 ELSE 0 END,
        ...
        'bill_no_processed', p_bill_no,
        'is_last_bill', p_is_last_bill,
