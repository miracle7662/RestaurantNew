CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_perform_checkout`(
    ...
    IN p_total_nights INT,          -- ✅ existing
    IN p_bill_no INT,               -- ✅ NEW: 0 = legacy, >0 = filter by bill_no
    IN p_is_last_bill TINYINT       -- ✅ NEW: 1 = final bill, triggers room checkout
)
sp_perform_checkout: BEGIN
    ...
    -- ============================================================================
    -- Insert or Update Checkout_Master (unchanged logic, with new status handling)
    -- ============================================================================
    ...
    -- ============================================================================
    -- Insert Checkout_Detail – with bill_no
    -- ============================================================================
    INSERT INTO Checkout_Detail (
        checkin_id, checkout_id, hotelid, room_id, room_number,
        room_category_id, room_category_name,
   ...
   WHERE cgfm.checkin_id = p_checkin_id
  AND (
      -- Legacy mode (single bill): room condition applies
      (p_bill_no = 0 AND (cgfm.room_id IS NULL OR FIND_IN_SET(cgfm.room_id, v_active_room_ids) > 0))
      OR
      -- Multi-bill mode: ignore room, only match bill_no
    ...
    SELECT JSON_OBJECT(
        ...
        'bill_no_processed', p_bill_no,      -- ✅ NEW
        'is_last_bill', p_is_last_bill,      -- ✅ NEW
