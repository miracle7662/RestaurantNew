...
exports.getOutletSettings = (req, res) => {
  try {
    ...
    const { role_level, } = req.user || {}; // assuming middleware adds user info

    // Validate required fields
    if (!id || !hotelid) {
      return res.status(400).json({ error: 'hii' });
    }

    let query = `
      SELECT 
        o.outletid, o.outlet_name, o.outlet_code, o.hotelid, o.market_id, o.phone, 
        o.email, o.website, o.address, o.city, o.zip_code, o.country, o.timezone, 
        o.start_day_time, o.close_day_time, o.next_reset_bill_date, o.next_reset_bill_days, 
        o.next_reset_kot_date, o.next_reset_kot_days, o.contact_phone, o.notification_email, 
        o.description, o.logo, o.gst_no, o.fssai_no, o.status, o.digital_order, 
        o.created_by_id, o.created_date, o.updated_by_id, o.updated_date,
    ...
    // If user is hotel_admin, restrict to their hotel only
    if (role_level === "hotel_admin" && hotelid) {
      query += " AND o.hotelid = ?";
      params.push(hotelid);
    }
    ...
    console.log('Constructed query:', query, 'with params:', params);
    const settings = db.prepare(query).get(...params);
    ...
    console.error(
      `Error fetching outlet settings for outletId=${req.params.id}, hotelId=${req.params.hotelid}:`,
      error.message
    );
