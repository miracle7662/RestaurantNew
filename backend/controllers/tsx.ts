...
const [formData, setFormData] = useState({
  ...
  const getTableButtonClass = (table: TableItem) => {
    let className = 'btn-outline-secondary'; // Default/Vacant (status 0)
    if (table.status === 1) {
      className = 'btn-success text-white'; // Occupied
    } else if (table.status === 2) {
      className = 'btn-danger text-white'; // Billed
  }

    if (selectedTable === table.table_name) {
      return `btn ${className} border border-primary border-3`;
  }
...
const fetchTableManagement = async () => {
  ...
  const handleTableClick = async (seat: string) => {
    ...
    const selectedTableRecord = tableItems.find(t => t.table_name === seat);

    if (selectedTableRecord) {
      const tableId = Number(selectedTableRecord.tablemanagementid || (selectedTableRecord as any).tablemanagementid);
      const deptId = Number(selectedTableRecord.departmentid) || null;
      const outletId = Number(selectedTableRecord.outletid) || null;
      ...
      // If table is Occupied (1) or Billed (2), fetch its running KOT data
      if (selectedTableRecord.status === 1 || selectedTableRecord.status === 2) {
        try {
          setLoading(true);
          const response = await getOpenBillByTableId(tableId);

          if (response?.success && response?.data) {
            const bill = response.data;
            setItems(bill.details || []);
            setCurrentBillId(bill.TxnID);
            setCustomerName(bill.CustomerName || '');
            setMobileNumber(bill.MobileNo || '');
            toast.success(`Loaded order for table ${seat}`);
          } else {
            // This can happen if status is 1 but bill was just settled/cancelled.
            // Clearing is a safe fallback.
            setItems([]); setCurrentBillId(null); setCustomerName(''); setMobileNumber('');
          }
        } catch (e: any) {
          toast.error(e?.message || 'Could not load table data.');
          setItems([]); setCurrentBillId(null); setCustomerName(''); setMobileNumber('');
        } finally {
          setLoading(false);
        }
      } else { // Table is Vacant (0), so start a new order
        setItems([]); setCurrentBillId(null); setCustomerName(''); setMobileNumber('');
      }
    }
  ...
  const handlePrintBill = async () => {
    if (!currentBillId) {
      toast.error("No active bill selected to print.");
      return;
    }

    try {
      setLoading(true);
      const response = await markAsBilled(currentBillId);

      if (response?.success) {
        toast.success("Bill finalized. Table status updated to 'Billed'.");

        // 1. Refresh table statuses to show red color
        ...
        if (printWindow) {
          ...
          if (contentToPrint) {
            // Using a simplified style for printing
            printWindow.document.write('<html><head><title>Bill</title><style>body { font-family: sans-serif; width: 300px; margin: 20px; } .text-end { text-align: right; } .d-flex { display: flex; } .justify-content-between { justify-content: space-between; } table { width: 100%; border-collapse: collapse; } th, td { padding: 4px; border: 1px solid #ccc; } </style></head><body>' + contentToPrint.innerHTML + '</body></html>');
        ...
        // 3. Clear the items from the panel and go back to table view
        setItems([]);
        ...
        toast.error(response?.message || "Failed to finalize bill.");
      }
    } catch (e: any) {
      toast.error(e?.message || "An error occurred while finalizing the bill.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrintAndSaveKOT = async () => {
    try {
      ...
      if (resp?.success) {
        ...
        // Refresh the table list to show the new 'occupied' status
        fetchTableManagement();
                                ...
                                assignedTables.map((table, tableIndex) => (
                                  table.table_name ? (
                                    <div key={tableIndex} className="p-1">
                                       <button
                                        className={getTableButtonClass(table)}
              ...
              <div className="mt-1">
                ...
                <div className="d-flex justify-content-center gap-2 mt-2">
                  ...
                  <button
                    ...
                    disabled={items.length === 0 || !currentBillId}
                  >
                    Print Bill
                  </button>
                </div>
