...
const simpleReports: Record<SimpleReportKey, SimpleReport> = {
  occupancy: {
    ...
    columns: ["#", "Room No", "Room Category", "Converted Category", "Guest", "Total Days", "Total Amt", "Discount %", "Pay Type", "Adults", "Pax", "Ex-Pax", "Child", "Driver"],
    rows: [],
  ...
  dailysell: {
    ...
    columns: ["#", "Date", "Room No", "Room Category", "Guest", "Nights", "Rate", "Tax", "Total Amt", "Pay Type"],
    rows: [],
  ...
  agent: {
    ...
    columns: ["#", "Agent Name", "Guest", "Room No", "Booking Date", "Total Amt", "Commission %", "Commission Amt"],
    rows: [],
    footerLabel: "Total Bookings:",
    footerCol: 5,
    footerAmtCol: 5,
...
const guestReport: GuestReport = {
  ...
  defaultFields: ["guest_name", "mobile", "email", "guest_type", "total_stays", "total_room_nights", "total_spent", "last_visit", "loyalty_level"],
};
...
function statusBadgeClass(value: string): string {
  ...
  if (v === "paid" || v === "gold" || v === "platinum") return "bg-success-subtle text-success-emphasis";
  if (v === "pending" || v === "new") return "bg-danger-subtle text-danger-emphasis";
  if (v === "partial" || v === "silver" || v === "bronze") return "bg-warning-subtle text-warning-emphasis";
  return "bg-secondary-subtle text-secondary-emphasis";
}

const STATUS_WORDS = ["paid", "pending", "partial", "gold", "silver", "bronze", "platinum", "new"];
...
export default function ReportsPage(): JSX.Element {
  ...
  const [hotelid] = useState(1); // TODO: replace with real hotel id

  const [guestRows, setGuestRows] = useState<GuestReportRow[]>([]);
  const [guestLoading, setGuestLoading] = useState(false);
  const [guestError, setGuestError] = useState<string | null>(null);

  const [simpleSelectedColumns, setSimpleSelectedColumns] = useState<Record<SimpleReportKey, string[]>>(() => {
  ...
  // ---------- fetchGuestReport ----------
  const fetchGuestReport = useCallback(async (params: {
    hotelid: number;
    fromDate: string;
    toDate: string;
    limit?: number;
  }): Promise<GuestReportRow[]> => {
    console.log("🚀 fetchGuestReport called with", params);
    try {
      ...
      // Extract array from various wrappers
      const extractArray = (obj: any): any[] => {
        if (Array.isArray(obj)) return obj;
        if (obj && typeof obj === "object") {
          const candidates = ["data", "rows", "result", "items", "records", "list"];
      ...
      // 🔥 If no data, return a dummy row for testing (remove later)
      if (rows.length === 0) {
        console.warn("⚠️ No data from API – using dummy data for testing");
        return [{
          guest_id: 1,
          ...
          company_credit_limit: 10000,
          company_credit_allowed: "Yes",
          unique_rooms_used: 2,
          room_numbers_used: "101, 102",
          room_categories_used: "Deluxe, Suite",
          room_details: "101 (Deluxe), 102 (Suite)",
          most_used_room: "101",
          preferred_room_category: "Deluxe",
          total_ldg_bills: 3,
          ldg_bill_numbers: "LDG001, LDG002",
          registration_numbers: "REG001",
          booking_references: "BK001",
          total_stays: 5,
          total_checkouts: 5,
          total_room_nights: 10,
          avg_stay_duration: 2,
          total_room_revenue: 5000,
          total_extra_charges: 200,
          total_child_charges: 100,
          total_driver_charges: 50,
          total_service_charge: 150,
          total_cess: 20,
          total_discounts_received: 300,
          total_cgst: 100,
          total_sgst: 100,
          total_igst: 0,
          total_spent: 5220,
          total_advance_paid: 2000,
          first_visit: "2026-01-01",
          last_visit: "2026-07-10",
          customer_lifecycle_days: 190,
          avg_amount_per_stay: 1044,
          loyalty_level: "Gold",
          total_payment_received: 5000,
          total_tips_given: 50,
  ...
  // ---------- useEffect for fetching guest report ----------
  useEffect(() => {
    console.log("🟢 useEffect triggered, activeReport =", activeReport);
    if (activeReport !== "guest") {
      console.log("⏭️ Skipping fetch because activeReport is not guest");
      return;
    }
  ...
  const refreshGuestReport = () => {
    ...
    fetchGuestReport({ hotelid, fromDate, toDate })
      .then(setGuestRows)
      .catch((err: unknown) => setGuestError(err instanceof Error ? err.message : "Failed to load guest report"))
  ...
  const toggleAllFields = () => {
    setSelectedFields((prev) => (prev.length === guestReport.fields.length ? [] : guestReport.fields.map((f) => f.key)));
  };
  ...
  const renderSimpleTable = (report: SimpleReport) => {
    ...
    const visibleIndices = visible.map((col) => report.columns.indexOf(col)).filter((idx) => idx !== -1);
      ...
      <table className="table table-hover align-middle mb-0">
            ...
            report.rows.map((row, i) => (
              <tr key={i}>
                {visibleIndices.map((idx) => (
                  <td key={idx} className="text-nowrap">{formatCell(row[idx])}</td>
                ))}
              </tr>
            ))
          )}
        ...
        <tfoot>
          <tr className="table-light fw-bold">
            <td colSpan={amtColIndex}>{report.footerLabel} {report.rows.length}</td>
          ...
          {guestColumns.length === 0 ? (
            ...
            guestRows.map((row, i) => (
              <tr key={row.guest_id ?? i}>
                {guestColumns.map((c) => (
                  <td key={c.key} className="text-nowrap">{formatCell(row[c.key as keyof GuestReportRow])}</td>
                ))}
              </tr>
            ))
          )}
  ...
  return (
    <div className="bg-white">
      ...
      <div className="d-flex align-items-center gap-3 p-3 border-bottom flex-wrap">
        {/* Report Dropdown - React controlled */}
        <div className="dropdown" style={{ position: 'relative' }}>
          <button
            className="btn btn-primary dropdown-toggle fw-semibold"
            type="button"
            onClick={() => setReportDropdownOpen(!reportDropdownOpen)}
            onBlur={() => setTimeout(() => setReportDropdownOpen(false), 150)}
          >
            {activeLabel}
          </button>
          {reportDropdownOpen && (
            <ul className="dropdown-menu show" style={{ display: 'block', position: 'absolute', top: '100%', left: 0, zIndex: 1000 }}>
              <li><h6 className="dropdown-header">Select Report</h6></li>
              {reportMenu.map((r) => (
                <li key={r.key}>
                  <button
                    className={`dropdown-item ${activeReport === r.key ? "active" : ""}`}
                    onClick={() => {
        ...
        <input type="date" className="form-control" style={{ width: 170 }} value={fromDate} onChange={(e) => setFromDate(e.target.value)} />
        <input type="date" className="form-control" style={{ width: 170 }} value={toDate} onChange={(e) => setToDate(e.target.value)} />
        ...
        <div className="ms-auto d-flex gap-2">
          ...
          <div className="dropdown" style={{ position: 'relative' }}>
            ...
            {exportDropdownOpen && (
              <ul className="dropdown-menu show dropdown-menu-end" style={{ display: 'block', position: 'absolute', top: '100%', right: 0, zIndex: 1000 }}>
                <li><button className="dropdown-item" onClick={() => { setExportDropdownOpen(false); /* handle export */ }}>Export as Excel</button></li>
                <li><button className="dropdown-item" onClick={() => { setExportDropdownOpen(false); /* handle export */ }}>Export as PDF</button></li>
              </ul>
            )}
          </div>

          <button className="btn btn-outline-primary" title="Refresh" onClick={activeReport === "guest" ? refreshGuestReport : undefined}>
      ...
      <div className="d-flex align-items-center justify-content-between px-3 pt-3 pb-2 flex-wrap gap-2">
        ...
        {activeReport === "guest" && (
          <div className="dropdown" style={{ position: 'relative' }}>
            ...
            {fieldDropdownOpen && (
              <div className="dropdown-menu show p-3" style={{ display: 'block', width: 360, maxHeight: 420, overflowY: 'auto', position: 'absolute', top: '100%', right: 0, zIndex: 1000 }}>
                <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                  <span className="small fw-bold text-uppercase text-muted">Choose columns</span>
                  <button className="btn btn-link btn-sm p-0" onClick={toggleAllFields}>
                    {selectedFields.length === guestReport.fields.length ? "Clear all" : "Select all"}
                  </button>
                </div>
          ...
          <div className="dropdown" style={{ position: 'relative' }}>
            ...
            {columnDropdownOpen && (
              <div className="dropdown-menu show p-3" style={{ display: 'block', width: 360, maxHeight: 420, overflowY: 'auto', position: 'absolute', top: '100%', right: 0, zIndex: 1000 }}>
                <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                  <span className="small fw-bold text-uppercase text-muted">Choose columns</span>
                  <button className="btn btn-link btn-sm p-0" onClick={() => toggleAllSimpleColumns(activeReport as SimpleReportKey)}>
                    {simpleSelectedColumns[activeReport as SimpleReportKey]?.length === currentSimpleReport.columns.length
                      ? "Clear all"
                      : "Select all"}
                  </button>
                </div>
                    ...
                    <div className="col" key={col}>
                      <div className="form-check">
                        <input
                          ...
                          type="checkbox"
                          id={`col_${col}`}
                          checked={(simpleSelectedColumns[activeReport as SimpleReportKey] || []).includes(col)}
                          onChange={() => toggleSimpleColumn(activeReport as SimpleReportKey, col)}
                        />
      ...
      <div className="table-responsive">
        {activeReport === "guest" ? renderGuestTable() : renderSimpleTable(currentSimpleReport!)}
      </div>
    </div>
  );
}