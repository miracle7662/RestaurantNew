...
const dailySummaryFields: FieldDef[] = [
  ...
  { key: "Total Tax Amount", label: "Tax Amount" },
  { key: "Total CGST", label: "CGST" },
  { key: "Total SGST", label: "SGST" },
  { key: "Total IGST", label: "IGST" },
    ...
    async (params: { hotelid: number; fromDate: string; toDate: string }): Promise<DailyBookingRow[]> => {
      ...
      return rawData.map((item: any) => {
        ...
        return {
          ...
          cgst: parseFloat(item['Total CGST'] ?? '0'),
          sgst: parseFloat(item['Total SGST'] ?? '0'),
          igst: parseFloat(item['Total IGST'] ?? '0'),
