...
import { tableManagementService, TableItem } from '@/services/TableManagementService';
...
const TableManagement: React.FC = () => {
  ...
  // Fetch table data using updated backend controller
  const fetchTableManagement = async () => {
    setLoading(true);
    try {
      const response = await tableManagementService.getTables();
      if (response.success) {
    ...
    debounce((value: string) => {
      ...
      const filtered = tableItems.filter((item) => {
        return (
          ...
          (item.outletname && item.outletname.toLowerCase().includes(value.toLowerCase()))
        );
      });
  ...
  const handleDeleteTable = async (table: TableItem) => {
    ...
    if (res.isConfirmed) {
      try {
        const response = await tableManagementService.deleteTable(table.tablemanagementid);
        if (response.success) {
...
const AddTableModal: React.FC<AddTableModalProps> = ({ show, onHide, onSuccess }) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const payload = {
        table_name,
        hotel_name: brands.find(b => b.hotelid === selectedBrand)?.hotel_name || '',
      ...
      const response = await tableManagementService.createTable(payload);
      if (response.success) {
...
const EditTableModal: React.FC<EditTableModalProps> = ({
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const response = await tableManagementService.updateTable(msttablemanagement.tablemanagementid, payload);
      if (response.success) {
