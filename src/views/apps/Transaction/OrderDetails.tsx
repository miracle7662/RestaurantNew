  ...
  const handleQuantityKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && (searchName || searchCode)) {
      ...
      if (matchedItem) {
        ...
        handleAddItem({ name: matchedItem.ItemName, price: matchedItem.price, revQty: 0, isBilled: 0, isNCKOT: 0, NCName: '', NCPurpose: '', alternativeItem: undefined, modifier: [] }, qty);
        setSearchCode('');
        setSearchName('');
        setQuantity('1');
        setShowNameDropdown(false);
                      ...
                      filteredItems.map((item, index) => (
                        <Col key={index}>
                          <Card
                            ...
                            onClick={() => handleAddItem({ name: item.ItemName, price: item.price, revQty: 0, isBilled: 0, isNCKOT: 0, NCName: '', NCPurpose: '', alternativeItem: undefined, modifier: [] }, parseInt(quantity) || 1)}
                            onMouseEnter={(e) => {
