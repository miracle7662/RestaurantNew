...
function OrderService() {
  return {
    ...
    getSavedKOTs: (params: { isBilled: number }) => {
      return HttpClient.get('/TAxnTrnbill/kots/saved', { params })
    },
  ...
  getUnbilledItemsByTable: (tableId: number) => {
    ...
    getBilledBillByTable: async (tableId: number) => {
  const res = await HttpClient.get(`/TAxnTrnbill/billed-bill/by-table/${tableId}`)
  return res.data
},
    ...
    /* Auth */
    verifyBillCreatorPassword: (payload: { password: string; txnId: string }) => {
      return HttpClient.post('/auth/verify-bill-creator-password', payload)
    },

    verifyCreatorPassword: (payload: { password: string }) => {
      return HttpClient.post('/auth/verify-creator-password', payload)
    },
  }
}
